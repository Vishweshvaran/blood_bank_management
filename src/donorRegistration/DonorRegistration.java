...
public class DonorRegistration extends JFrame {
	...
	Choice ct,blg;
	Button sub;
	public DonorRegistration() throws SQLException {
		// Frame Layout
		setLayout(null);
		setBounds(100, 200, 600, 600); // ( setSize + setLocation )
		setBackground(MainPage.backGroundColor);
		setVisible(true);
		setResizable(false);
		...
		// heading
		heading = new Label("DONOR REGISTRATION FORM");
		...
		sub.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(fnmat.matches() && lnmat.matches() && ((m.getState() == true) || (f.getState() == true)) &&  !ct.getSelectedItem().equals("select") && emat.matches() &&  phmat.matches() &&  !ct.getSelectedItem().equals("select")) {
					try {		
